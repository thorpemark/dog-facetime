-- Focal zoom for portrait crop framing (run after migration_focal_point.sql)
-- Adds zoom-out factor per media asset so together / landscape shots can include more width.

ALTER TABLE media_assets
  ADD COLUMN IF NOT EXISTS focal_zoom REAL NOT NULL DEFAULT 1
    CHECK (focal_zoom >= 1 AND focal_zoom <= 4);
...
-- Patch get_memorial_public to include focal_zoom
...
-- Patch get_memorial_for_edit to include focal_zoom
...
-- Patch register_media_asset to accept optional focal zoom
