-- 2D crop dimensions for portrait framing (run after migration_focal_zoom.sql)
-- Adds optional normalized crop width/height so framing can be adjusted on both axes.
-- When NULL, crop size is derived from focal_zoom (legacy behavior).

ALTER TABLE media_assets
  ADD COLUMN IF NOT EXISTS focal_crop_w REAL
    CHECK (focal_crop_w IS NULL OR (focal_crop_w >= 0.08 AND focal_crop_w <= 1)),
  ADD COLUMN IF NOT EXISTS focal_crop_h REAL
    CHECK (focal_crop_h IS NULL OR (focal_crop_h >= 0.08 AND focal_crop_h <= 1));
...
GRANT EXECUTE ON FUNCTION update_media_focal_point(TEXT, UUID, REAL, REAL, REAL, REAL, REAL) TO anon, authenticated;

-- Patch get_memorial_public to include crop dimensions
...
-- Patch get_memorial_for_edit to include crop dimensions
...
-- Patch register_media_asset to accept optional crop dimensions
...
GRANT EXECUTE ON FUNCTION register_media_asset(TEXT, UUID, TEXT, TEXT, INT, TEXT, REAL, REAL, REAL, REAL, REAL) TO anon, authenticated;
