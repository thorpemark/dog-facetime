-- Fine rotation per orientation for call framing (run after migration_focal_orientation.sql)
-- focal_rotation_deg = portrait phone framing; landscape_focal_rotation_deg = landscape / desktop.

ALTER TABLE media_assets
  ADD COLUMN IF NOT EXISTS focal_rotation_deg REAL NOT NULL DEFAULT 0
    CHECK (focal_rotation_deg >= -15 AND focal_rotation_deg <= 15),
  ADD COLUMN IF NOT EXISTS landscape_focal_rotation_deg REAL
    CHECK (
      landscape_focal_rotation_deg IS NULL OR
      (landscape_focal_rotation_deg >= -15 AND landscape_focal_rotation_deg <= 15)
    );
...
CREATE OR REPLACE FUNCTION update_media_focal_point(
  ...
  p_focal_rotation_deg REAL DEFAULT 0,
  ...
  p_landscape_focal_rotation_deg REAL DEFAULT NULL
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET
    ...
    END,
    focal_rotation_deg = LEAST(15, GREATEST(-15, COALESCE(p_focal_rotation_deg, 0))),
    ...
    END,
    landscape_focal_rotation_deg = CASE
      WHEN p_landscape_focal_rotation_deg IS NULL THEN NULL
      ELSE LEAST(15, GREATEST(-15, p_landscape_focal_rotation_deg))
    END
  ...
  RETURN json_build_object(
    ...
    'focal_rotation_deg', v_asset.focal_rotation_deg,
    ...
    'landscape_focal_rotation_deg', v_asset.landscape_focal_rotation_deg
  );
END;
$$;

GRANT EXECUTE ON FUNCTION update_media_focal_point(TEXT, UUID, REAL, REAL, REAL, REAL, REAL, REAL, REAL, REAL, REAL, REAL, REAL, REAL) TO anon, authenticated;
          ...
          'media', (
            SELECT COALESCE(json_agg(
              json_build_object(
                ...
                'focal_rotation_deg', ma.focal_rotation_deg,
                ...
                'landscape_focal_rotation_deg', ma.landscape_focal_rotation_deg
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      ELSE LEAST(1, GREATEST(0.08, p_focal_crop_h))
    END,
    LEAST(15, GREATEST(-15, COALESCE(p_focal_rotation_deg, 0))),
    CASE
      WHEN p_landscape_focal_x IS NULL THEN NULL
      ELSE LEAST(1, GREATEST(0, p_landscape_focal_x))
    END,
...
GRANT EXECUTE ON FUNCTION register_media_asset(TEXT, UUID, TEXT, TEXT, INT, TEXT, REAL, REAL, REAL, REAL, REAL, REAL, REAL, REAL, REAL, REAL, REAL, REAL) TO anon, authenticated;
